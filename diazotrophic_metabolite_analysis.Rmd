...
smiles2fp <- function(smiles_vec) {
  ...
  if (!all(ok)) message(sum(!ok), " invalid SMILES dropped")
  mols <- mols[ok]

  fp_set <- lapply(mols, rcdk::get.fingerprint,
                   type = "standard", fp.mode = "bit")
...
`%notin%` <- Negate(`%in%`)
if (!exists("pred_u", inherits = FALSE)) {

  message("Re‑computing unknown‑compound predictions …")
  ...
  pred_u <- purrr::map_dfr(seq_along(fp_q), function(i) {
    ...
    best <- which.max(sims)                       # nearest neighbour
